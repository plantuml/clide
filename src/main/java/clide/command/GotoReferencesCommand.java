 ...
 * everywhere it's actually used). includeDeclaration is false: the
 * declaration itself is already known (it's this command's own input), so
 * only real usages are wanted - with includeDeclaration true, a never-called
 * method would still report one "reference" (its own declaration), defeating
 * the point of asking "is this actually called anywhere".
 */
