...
/**
 * Harmonized front end over goto_references: same <symbol> parameter and the
 * same underlying pipeline (see GotoPositionCommand.goToAndFormat()), with an
 * added leading <what> parameter for naming symmetry with
 * FindDeclarationCommand. Unlike find_declaration, <what> does not select
 * between two different LSP requests here - textDocument/references is sent
 * either way, since jdtls answers "who uses this" the same way regardless of
 * whether the symbol being searched is a method or a type. Its own literal
 * value is still checked (typo protection, consistent with find_declaration),
 * but - deliberately, to avoid an extra jdtls round trip just to find out -
 * it is not cross-checked against the actual kind of symbol found at
 * <symbol>'s position. goto_references is unchanged and keeps working
 * exactly as before; this is purely an additional, more consistently named
...
public class FindReferenceCommand extends Command {
	...
	@Manual("""
			...
			DESCRIPTION
				...
				same request, same result formatting as goto_references.
				...
				textDocument/references does not itself need. A
				harmonized, more consistently named front end over
				goto_references, which is unaffected and keeps working
				exactly as before.
			...
			SEE ALSO
				find_declaration(1), goto_references(1),
	...
	public CommandResult executeCommand(final ClideContext context, final String... params) {
		...
		return GotoPositionCommand.goToAndFormat(context, "find_reference", "textDocument/references", params[1],
